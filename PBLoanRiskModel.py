...
# Load the dataset
try:
    data = pd.read_csv('PB_Train_data.csv')
    print("PostBank Dataset loaded successfully!")
    print(f"Dataset shape: {data.shape}")
except Exception as e:
    print(f"Error loading PostBank dataset: {e}")
    exit()
...
print("\nTraining the PostBank model...")
model.fit(X_train, y_train)

# Evaluate the model
y_pred = model.predict(X_test)
accuracy = accuracy_score(y_test, y_pred)
print(f"\nPostBank Model Accuracy: {accuracy:.4f}")

# Save the model
model_filename = 'PB_loan_risk_assessment_model.pkl'
joblib.dump(model, model_filename)
print(f"\nPostBank Model saved as {model_filename}")
