...
# Load the dataset
try:
    data = pd.read_csv('SB_Train_data.csv')
    print("SandardBank Dataset loaded successfully!")
    print(f"Dataset shape: {data.shape}")
except Exception as e:
    print(f"Error loading StandardBank dataset: {e}")
    exit()
...
print("\nTraining the StandardBank model...")
model.fit(X_train, y_train)

# Evaluate the model
y_pred = model.predict(X_test)
accuracy = accuracy_score(y_test, y_pred)
print(f"\nStandardBank Model Accuracy: {accuracy:.4f}")

# Save the model
model_filename = 'SB_loan_risk_assessment_model.pkl'
joblib.dump(model, model_filename)
print(f"\nStandardBank Model saved as {model_filename}")
